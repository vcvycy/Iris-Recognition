...
namespace osiris
{
    ...
    */
    ...
    public :
			  //cjf
        int init(std::string& fileName, int minIrisDiameter , int minPupilDiameter , int maxIrisDiameter , int maxPupilDiameter,
        ...
        */
        ...
        //特征提取255*6=1530个0/1值,by cjf
				/*
				  参数[in]: Application Points的x,y坐标
					参数[out]:特征值，长度为appPnt.size()*2*Garbor.size=255*2*3=1530
				*/
				void  extractFeature(std::vector<int>& appPnts_x,std::vector<int>& appPnts_y,int nFilterNum,int shitedPixels,std::string& feature);
    private :
