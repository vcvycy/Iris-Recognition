...
{    
                                         ...
                                         int maxPupilDiameter )
    {
			  ...
			  SHOW_TIME("int 10^8");
				...
				TIME_START
				  float sumf=0;
				  for (float i=0.0;i<=100000;i+=0.01){
						 float p=i*i;
					   sumf+=p-(int)p;
						 //printf("%f \n",p-(int)p);
					}
					printf("sumf=%f\n",sumf);
			  SHOW_TIME("float 10^7");
			  */
				TIME_START
        // Check arguments
        //////////////////
...
// Fill the holes in an area surrounding pupil
        ...
        int scale=1;
				...
				IplImage *clone_src=cvCreateImage(cvSize(pSrc->width/scale,pSrc->height/scale),pSrc->depth,pSrc->nChannels);
				cvResize(pSrc,clone_src);

				cvSetImageROI(clone_src,cvRect((rPupil.getCenter().x-3.0/4.0*maxIrisDiameter/2.0)/scale,
                                       (rPupil.getCenter().y-3.0/4.0*maxIrisDiameter/2.0)/scale,
                                       3.0/4.0*maxIrisDiameter/scale,
                                       3.0/4.0*maxIrisDiameter/scale)) ;
TIME_START	
				fillWhiteHoles(clone_src,clone_src) ;  // by cjf
				cvResetImageROI(clone_src) ;
SHOW_TIME("FillWhiteHoles");
...
TIME_START
        ...
        vector<CvPoint> pupil_accurate_contour ;
				                                              findContour(clone_src,
                                                             rPupil.getCenter(),
                                                             theta,
                                                             rPupil.getRadius()-20,
                                                             rPupil.getRadius()+20,
...
TIME_START
												...
												const vector<CvPoint> & rIrisCoarseContour )
    {
        ...
        for ( int j = 0 ; j < pDst->width ; j++ )
        {
            ...
            {    
				...
				SHOW_TIME("normalize from contour");
    }
    ...
    //void  OsiProcessings::getFeatures(
    ...
    {   
        ...
        for ( int s = -shift ; s <= shift ; s++ )
        {
            ...
            //相同的特征点/所有特征点,cvSum返回4个double值，不同图像通道
						float mean = (cvSum(result).val[0])/(cvSum(mask).val[0]) ;
						printf(" shift[%d]  score %f\n",s,mean);
    ...
    {        
        ...
        fillWhiteHoles(resized,filled) ;
				SHOW_TIME("FillWhiteHoles");
        //IplImage* filled=cvCloneImage(resized);
    ...
    //    TIME_START  
        ...
        cvReleaseImage(&mask) ;
        cvReleaseImage(&result) ;        
      //  SHOW_TIME("Fill White Holes");
				/*
        static int cnt=0;
				cnt++;
				char filename[50];
				sprintf(filename,"BeforeFill_%d.jpg",cnt);cvSaveImage(filename,pSrc);
				sprintf(filename,"AfterFill_%d.jpg",cnt);cvSaveImage(filename,pDst);*/
    } // end of function
    ...
    void OsiProcessings::findContour ( const IplImage * pSrc ,
                                                  CvPoint  rCenter ,
                                                  const vector<float> & rTheta ,
                                                        int minRadius ,
                                                        int maxRadius ,
																									vector<CvPoint>& contour,
																									int   scale,   //表示pSrc被缩放了scale倍。但后面的参数却还是原图像的参数
                                                  const IplImage * pMask
																									)
    {
			  //TIME_START
        ...
        IplImage * unwrapped = unwrapRing(pSrc,rCenter,minRadius,maxRadius,rTheta) ;
				...
				//debug
				/*
        printf("pSrc %d*%d upwarpped %d*%d\n",pSrc->width,pSrc->height,unwrapped->width,unwrapped->height);
        static int pp=0;
				char s[20];
				sprintf(s,"pSrc_%d.jpg",pp);cvSaveImage(s,pSrc);
				sprintf(s,"unwrap_%d.jpg",pp);cvSaveImage(s,unwrapped);
				if (pp++==4)exit(0);*/
				// Smooth image
