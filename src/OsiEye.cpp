...
namespace osiris
{
    ...
    void OsiEye::saveImage ( const string & rFilename , const IplImage * pImage )
    {
        ...
        if ( ! pImage )
        {
            throw runtime_error("Cannot save image " + rFilename + " because this image is not built") ;
				...
				cout<<"保存图片:"<<rFilename.c_str()<<endl;
    ...
    void OsiEye::normalize ( int rWidthOfNormalizedIris , int rHeightOfNormalizedIris )
    {
       ...
       //OsiIris v4.0和v4.1在normalize方式上的改进。cjf 
				//此处为原始图片的normalize/cjf
    ...
    void OsiEye::encode ( const vector<CvMat*> & rGaborFilters )
    {
        ...
        //mpIrisCode Size=(512,64*6)
				// Encode
        OsiProcessings op ;
        op.encode(mpNormalizedImage,mpIrisCode,rGaborFilters) ;
		}

		void  OsiEye::extractFeature(std::vector<int>& appPnts_x,std::vector<int>& appPnts_y,int nFilterNum,int shiftedPixels,std::string& feature){
		    feature="";
			  int height=mpIrisCode->height/nFilterNum;       
				int width =mpIrisCode->width;
				uchar* data=(uchar*)mpIrisCode->imageData;
				
				for (int i=0;i<nFilterNum;i++){
				  for (int pid=0;pid<appPnts_x.size();pid++){
						 int x=appPnts_x[pid]+height*i;
					   int y=appPnts_y[pid]+shiftedPixels;
						 uchar tmp;//=data[y+x*width];
						 if(y>=0 && y<width) tmp=data[y+x*width];else tmp=0;
						 if (tmp==0){
						   feature+='0';
						 }else
						 if (tmp==255){
						   feature+='1';
						 }else {printf("OsiExtractFeature Erro!\n");exit(-100);}

					}
				}
			//std::cout<<getFeature()->length()<<"-- "<<*getFeature()<<std::endl;
		}
    ...
    float OsiEye::match ( OsiEye & rEye , const CvMat * pApplicationPoints )
    {
        ...
        mpNormalizedMask=rEye.mpNormalizedMask=NULL; //cjf 将所有mask去掉 
				if ( ! mpNormalizedMask )
        {
    ...
    int OsiEye::init(std::string& filename, int minIrisDiameter , int minPupilDiameter , int maxIrisDiameter , int maxPupilDiameter,
				int rWidthOfNormalizedIris , int rHeightOfNormalizedIris ,std::vector<CvMat*> & rGaborFilters){
