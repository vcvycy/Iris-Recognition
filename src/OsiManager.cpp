...
namespace osiris
{
    ...
    void OsiManager::loadConfiguration ( const string & sConfigPath)
    {
        ...
        // Open the file
        ifstream file((sPath+"conf.ini").c_str(),ifstream::in) ;
        
        if ( ! file.good() )
            throw runtime_error("[!]无法读取配置文件 " + sPath + "conf.ini") ;
    ...
    void OsiManager::showConfiguration ( )
    {
        ...
        {
					  //统计application个数：当矩阵值为255表示是application point，0表示不是，故求出矩阵和再除以255(即max_val)
            double max_val ;
            cvMinMaxLoc(mpApplicationPoints,0,&max_val) ;
            cout << "- " << cvSum(mpApplicationPoints).val[0]/max_val << " application points"<<max_val << endl ;
        }
    ...
    void OsiManager::loadApplicationPoints ( )
    {
        ...
        {    
            // Get the coordinates
            file >> i ; file >> j ;
            mAppPnts_x.push_back(i);
						mAppPnts_y.push_back(j);
            // Set pixel to "on"
    ...
    void OsiManager::processOneEye ( const string & rFileName , OsiEye & rEye )
    {
        ...
        cout<<"short_name:"<<short_name<<" "<<rFileName<<endl;
        ...
        // Save mask
				printf("mOutputDIrmask");cout<<mOutputDirMasks<<endl;
        if ( mOutputDirMasks != "" )
        {
		...
		int OsiManager::extractFeature(std::string& rFileName,std::vector<int> &shift,std::vector<std::string>& vFeature){
			  OsiEye rEye;
				//以下改自processOneEye
				int ret=rEye.init(rFileName,mMinIrisDiameter,mMinPupilDiameter,mMaxIrisDiameter,mMaxPupilDiameter,
						       mWidthOfNormalizedIris,mHeightOfNormalizedIris,mGaborFilters);
				if (ret!=0) return ret;
				vFeature.clear();
				for (int i=0;i<shift.size();i++){
					vFeature.push_back(string(""));
				  rEye.extractFeature(mAppPnts_x,mAppPnts_y,6,shift[i],vFeature[i]);
				}
				return 0;
		}
    ...
    {   
        ...
        if ( mProcessMatching && mOutputFileMatchingScores != "" )
        {   
            printf("Match Score saved to %s\n",mOutputFileMatchingScores.c_str());
            try 
            {
            ...
            catch ( exception & e )
            {
				...
				/*添加以下代码，使之两两匹配: 取消了normalized mask,会使score变大*/
        OsiEye myEye[20];
        for (int i=0;i< mListOfImages.size();i++){
				   cout<<"处理第"<<i<<"个眼,并将与前面的两两匹配,不处理MASK"<<endl;
           processOneEye(mListOfImages[i],myEye[i]);
					 string si;
					 int shifted=0;
					 //myEye[i].extractFeature(mAppPnts_x,mAppPnts_y,6,0,si);
					 for (int j=0;j< i;j++){
					   float score=myEye[i].match(myEye[j],mpApplicationPoints);
                            result_matching << mListOfImages[i] << " " ;
                            result_matching << mListOfImages[j] << " " ;
                            result_matching << score << endl ; 
					   string sj;
						 myEye[i].extractFeature(mAppPnts_x,mAppPnts_y,6,shifted,si);
						 myEye[j].extractFeature(mAppPnts_x,mAppPnts_y,6,0,sj);
						 int same_cnt=0;
						 for (int cid=0;cid<si.length();cid++) same_cnt+= (si.at(cid) == sj.at(cid));
						 result_matching<<1.0-1.0*same_cnt/si.length()<<endl;
					 }
           result_matching<<endl;
				}
        /*  ******/
				/* 以下为原先代码 
			  for ( int i = 0 ; i < mListOfImages.size() ; i++ )
            ...
            try
            {
                ...
                if ( mProcessMatching && (i<mListOfImages.size()-1) )
                {
                    ...
                    {                    
                        try
                        {
                            result_matching << mListOfImages[i-1] << " [CMP]" ;
                            result_matching << mListOfImages[i] << " " ;
                            result_matching << score << endl ;
                        }
                        catch ( exception & e )
